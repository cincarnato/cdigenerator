...
class PropertyGenerator {
    ...
    function __construct(\CdiGenerator\Entity\Property $property, \Zend\Code\Generator\ClassGenerator $classGenerator) {
        //INIT
        $this->property = $property;
    ...
    protected function generateAnnotation() {
        ...
        switch ($this->getProperty()->getType()) {
            ...
            case "date":
                $tagForm = ($tagForm) ? $tagForm : \CdiGenerator\FormAnnotation::DATE($this->getProperty());
                $tagDoctrine = \CdiGenerator\DoctrineAnnotation::DATE($this->getProperty());
                break;

            case "datetime":
                $tagForm = ($tagForm) ? $tagForm : \CdiGenerator\FormAnnotation::DATETIME($this->getProperty());
                $tagDoctrine = \CdiGenerator\DoctrineAnnotation::DATETIME($this->getProperty());
                break;

            case "time":
                $tagForm = ($tagForm) ? $tagForm : \CdiGenerator\FormAnnotation::TIME($this->getProperty());
         ...
         if ($this->getProperty()->getPrimarykey()) {
              $tagForm = array_merge($tagForm,\CdiGenerator\DoctrineAnnotation::ID());
         }
         
          //CHECK GeneratedValue = AUTO
         if ($this->getProperty()->getAutoGeneratedValue()) {
              $tagForm = array_merge($tagForm,\CdiGenerator\DoctrineAnnotation::AUTOGENERATEDVALUE());
         }
