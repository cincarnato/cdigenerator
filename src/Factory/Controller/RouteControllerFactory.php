...
class RouteControllerFactory implements FactoryInterface {
    ...
    public function __invoke(ContainerInterface $container, $requestedName, array $options = NULL) {
        ...
        $grid = $container->build("CdiDatagridDoctrine", ["customOptionsKey" => "CdiGenerator_Route"]);
        $grid->setTemplate("ajax");
        $grid->setId("CdiGrid_Route");
        $em = $container->get('doctrine.entitymanager.orm_cdigenerator');
        return new \CdiGenerator\Controller\RouteController($em, $grid);
    }
